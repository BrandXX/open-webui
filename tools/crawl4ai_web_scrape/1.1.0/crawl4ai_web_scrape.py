...
class Tools:
    ...
    async def web_scrape(
        ...
        try:
            ...
            # Add safety check for markdown content
            if not markdown:
                return "No content was retrieved from the webpage."

            # Limit markdown size before regex processing to prevent memory issues
            max_size_for_processing = 10 * 1024 * 1024  # 10MB limit
            if len(markdown) > max_size_for_processing:
                markdown = markdown[:max_size_for_processing]
