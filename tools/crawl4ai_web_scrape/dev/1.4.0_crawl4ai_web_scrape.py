...
class Tools:
    ...
    async def generate_queries(self, user_query: str) -> List[str]:
        """
        If OLLAMA_MODEL is empty, we fallback to trivial expansions.
        Otherwise, we call Ollama with the user-defined model name.
        """
        model_name = (self.valves.OLLAMA_MODEL or "").strip()
        if not model_name:
            return [user_query, f"{user_query} info", f"{user_query} research"]

        # NEW, REVISED SYSTEM INSTRUCTIONS:
        system_instructions = (
